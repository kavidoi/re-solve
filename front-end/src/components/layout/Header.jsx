    ...
    <header className="bg-white dark:bg-gray-800 shadow-sm sticky top-0 z-40 border-b border-accent dark:border-gray-700">
      <nav className="container mx-auto px-4 py-3 flex justify-between items-center">
        <Link to="/dashboard" className="flex items-center space-x-2">
          ...
          <span className="text-xl font-bold text-gray-900 dark:text-white">Re:solve</span>
        </Link>
        ...
        <div className="flex items-center space-x-4">
          ...
          {isAuthenticated ? (
            ...
            <>
              <Link to="/login" className="text-gray-600 dark:text-gray-300 hover:text-primary dark:hover:text-indigo-400 px-3 py-2 rounded-md text-sm font-medium">Login</Link>
              <Link to="/register" className="text-white bg-primary hover:bg-primary-dark px-4 py-2 rounded-md text-sm font-medium shadow-sm transition duration-150 ease-in-out">Register</Link>
            </>
          )}
        </div>
      </nav>
